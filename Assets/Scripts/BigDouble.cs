...
namespace BreakInfinity
{
    ...
    public struct BigDouble : IFormattable, IComparable, IComparable<BigDouble>, IEquatable<BigDouble>
    {
        ...
        private static class BigNumber
        {
            ...
            // ** CANVIAT PEL MEU PROJECTE ** EL MÈTODE PER PODER UTILITZAR EL FORMAT 'N'. Provisional (?
            private static string FormatFixed(BigDouble value, int places, bool useThousandsSeparator = false)
            {
                if (places < 0)
                {
                ...
                {
                    var formatString = useThousandsSeparator ? $"N{places}" : $"F{places}";
                    result = value.ToDouble().ToString(formatString, CultureInfo.InvariantCulture);

                    // Si estem usant format 'F', convertim de nou perquè ToString amb "N" pot haver afegit separadors
                    if (!useThousandsSeparator)
                    {
                        result = ToFixed(value.ToDouble(), places);
                    }

                    return result;
                }

                // Afegir separadors de milers si es demana format 'N'
                if (useThousandsSeparator)
                {
                    ...
                    // Afegir comes cada 3 dígits
                    var withSeparators = "";
                    var isNegative = integerPart.StartsWith("-");
                    ...
                    result = (isNegative ? "-" : "") + withSeparators;
